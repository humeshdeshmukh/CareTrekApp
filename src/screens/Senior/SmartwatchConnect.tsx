...
const SmartwatchConnect: React.FC<SmartwatchConnectProps> = ({
  visible,
  onClose,
  onDeviceConnected,
  onDataReceived
  ...
  const theme = colors; // For backward compatibility
  ...
  const connectToDevice = async (device: Device) => {
    try {
      ...
      const deviceConnection = await device.connect({ autoConnect: false });
      
      // Discover all services and characteristics
      const connectedDevice = await deviceConnection.discoverAllServicesAndCharacteristics();
      
      setConnectedDevice(connectedDevice);
      setConnectionStatus('connected');
      
      // Start monitoring for notifications (if your device supports it)
      // This is where you would set up notifications for heart rate, steps, etc.
      // The exact implementation depends on your smartwatch's BLE specifications
      
      // For demonstration, we'll simulate some data
      simulateDeviceData(connectedDevice);
  ...
  const simulateDeviceData = (device: Device) => {
    // Simulate heart rate data (60-100 bpm)
    const heartRate = Math.floor(Math.random() * 40) + 60;
    
    // Simulate steps (0-20000)
